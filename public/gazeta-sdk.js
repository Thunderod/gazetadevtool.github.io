// Global Gazeta Namespace for Zero-Latency Pre-fetching
window.Gazeta = window.Gazeta || {
    _adPool: [],
    _isFetching: false,
    _config: null,
    
    /**
     * Initializes the SDK and pre-fetches the ad pool into local cache.
     * Call this on the app splash screen or initialization phase.
     */
    async init(config) {
      this._config = config;
      await this._restockPool();
    },
  
    async _restockPool() {
      if (this._isFetching || !this._config || !this._config.appId) return;
      ...
      try {
        ...
        if (!response.ok) throw new Error("Failed to fetch ad pool");
        
        if (Array.isArray(jsonArray) && jsonArray.length > 0) {
          // Preload media assets invisibly into the browser's HTTP cache
          jsonArray.forEach(ad => this._preloadAsset(ad.media_url, ad.file_type));
          this._adPool.push(...jsonArray);
        }
      } catch (e) {
        console.error("Gazeta Pre-fetching Error:", e);
      } finally {
        this._isFetching = false;
      }
    },
  
    _preloadAsset(url, type) {
    ...
    /**
     * Retrieves the next cached ad instantly, or fetches one if the pool is empty.
     */
    async getNextAd(fallbackConfig) {
      // Allow the component to supply its own config if init() was never called
      if (!this._config && fallbackConfig) {
         this._config = fallbackConfig;
      }
      
      // If pool is empty, block and fetch now
      if (this._adPool.length === 0) {
        await this._restockPool();
      }
      
      // Pop the next pre-cached ad (0ms latency!)
      const ad = this._adPool.shift();
      
      // Background restock if running low
      if (this._adPool.length < 2 && this._config) {
        this._restockPool(); // deliberately not awaited
      }
